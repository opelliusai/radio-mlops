...
from src.scripts import monitoring_healthcheck
import os
...
def healthcheck_task(**context):
    status, message_objet, message_corps = monitoring_healthcheck.global_heathcheck()
...
def check_status(**context):
    ...
    status = task_instance.xcom_pull(task_ids='task_healthcheck', key='status')
...
with DAG(
    ...
    # Tâche de Healthcheck
    task1_healthcheck = PythonOperator(
        task_id='task_healthcheck',
        python_callable=healthcheck_task,
    ...
    task2_email = EmailOperator(
        ...
        to=os.getenv('AIRFLOW__SMTP__SMTP_MAIL_TO'),
    ...
    task1_healthcheck >> check_status_task >> [task2_email, task3_skip_email]
