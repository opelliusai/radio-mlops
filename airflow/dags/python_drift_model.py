...
def model_drift_task(**context):
    status, message_objet, message_corps = monitoring_drift.model_drift()
...
def check_status(**context):
    ...
    status = task_instance.xcom_pull(task_ids='task_model_drift', key='status')
...
with DAG(
    ...
    task_model_drift = PythonOperator(
        task_id='task_model_drift',
        python_callable=model_drift_task,
    ...
    # Flux d'exécution
    task_model_drift >> check_status_task
    check_status_task >> task_email
    check_status_task >> task_skip_email

    check_status_task.set_upstream(task_model_drift)
